...
Now, we remove the presence of the floor functions by using Lemma above. This yields
...
\begin{proof}
For any prime ideal $\mathfrak{p}$, there are $\bigl[H(N)-H\bigl(\frac{N}{\f{N(p)}}\bigr)\bigr]^n$ products of ideals with norm at most $N$ that are not divisible by $\f{N(p)}$. Hence, there are  $H(N)^n-\bigl[H(N)-H\bigl(\frac{N}{\f{N(p)}}\bigr)\bigr]^n$ products of ideals with norm at most \(N\) where at least one of them is divisible by $\f{N(p)}$. Therefore, there are 
$$H(N)^{nm}-\Big[H(N)^n - \Big(H(N) - H\Big(\frac{N}{\f{N(p)}}\Big)\Big)^n\Big]^m$$
ordered $m$-tuples of products of ideals, each of which has norm at most $N$ and is not divisible by $\f{N(p)}$.
	
Then since divisibility by finitely many primes yields independent events, we conclude that $$T(l,N)=\prod_{i=1}^{l} \Big[H(N)^{nm}- \Big(H(N)^n - \Big(H(N) - H\Big(\frac{N}{\f{N(p}_i)}\Big)\Big)^n \Big)^m \Big].$$
	
Next, we can compute the probability that the GCD of $m$ products of random ideals with norm at most $N$ are not divisible by the first $l$ prime ideals:
$$\frac{T(l,N)}{H(N)^{nm}} = \prod_{i=1}^{l} \Big[1 - \Big(1 - \Big(1 - \frac{1}{H(N)} H\Big(\frac{N}{\f{N(p)}}\Big)\Big)^n \Big)^m \Big].$$
	
\noindent Using the estimate for $H(n)$ from \cite{marcus_number_2012} then gives us
\begin{align}
\frac{T(l,N)}{H(N)^{mn}}=\prod_{i=1}^{l} \Big[1 - \Big(1 - \Big(1 - \frac{1}{\f{N}(\f{p}_i)} + O\Big(\frac{1}{N^{\epsilon}}\Big) \Big)^n \Big)^m\Big]\\
=\prod_{i=1}^{l} \Big[1 - \Big(1 - \Big(1 - \frac{1}{\f{N}(\f{p}_i)}\Big)^n \Big)^m \Big] + O\Big(\frac{1}{N^{\epsilon}}\Big).
\end{align}
Letting $N\to\infty$ now gives us the desired result. 
\end{proof}
...
\section{k-gcd prototype}
For any fixed \(k\geq 1\), prime \(p\), and sufficiently large integer \(N\geq 0\):
	\begin{enumerate}
		\item There are \(\lfloor \frac{N}{p} \rfloor\) positive integers at most \(N\) and divisible by \(p\).
		\item There are \(N-\lfloor \frac{N}{p} \rfloor\) positive integers at most \(N\) and not divisible by \(p\).
		\item There are \((N-\lfloor \frac{N}{p} \rfloor)^n\) products of \(n\) positive integers each of which is at most \(N\) and not divisible by \(p\).
		\item There are \(nH_k=\binom{n+k-1}{k}\) integer solutions to \(p^k=p^{x_1}p^{x_2}\mathellipsis p^{x_n}\).
		\item There are \(\sum_{a=0}^{k-1}\lfloor\frac{N}{p^a}\rfloor_nH_a\) ways to write a product of \(n\) positive integers at most \(N\) that is divisible by a non negative integer power of \(p\) up to \(k-1\).
		\item There are \((N-\lfloor \frac{N}{p} \rfloor)^n(\sum_{a=0}^{k-1}\lfloor\frac{N}{p^a}\rfloor_nH_a)\) ways to write products of \(n\) positive integers each of which is at most \(N\) and not divisible by a non negative integer power of \(p\) up to \(k-1\).
		\item There are \(N^n-[(N-\lfloor \frac{N}{p} \rfloor)^n(\sum_{a=0}^{k-1}\lfloor\frac{N}{p^a}\rfloor_nH_a)]\) ways to write products of \(n\) positive integers each of which is at most \(N\) and and at least one of them is divisible by a non negative integer power of \(p\) up to \(k-1\).
		\item There are \(N^{mn}-(N^n-[(N-\lfloor \frac{N}{p} \rfloor)^n(\sum_{a=0}^{k-1}\lfloor\frac{N}{p^a}\rfloor_nH_a)])^m\), ways to write \(m\)-tuples of products of \(n\) positive integers each of which is at most \(N\) and not divisible by a non negative integer power of \(p\) up to \(k-1\).
	\end{enumerate}
	Since divisibility by finitely many primes are independent events, we conclude that, \[T(l,N)=\prod_{i=1}^{l}[N^{mn}-(N^n-[(N-\lfloor \frac{N}{p_i} \rfloor)^n(\sum_{a=0}^{k-1}\lfloor\frac{N}{p_i^a}\rfloor_nH_a)])^m]\]
	
	Next, we can compute the probability that the GCD of \(m\) products of random positive integers at most \(N\) are not divisible by primes in  \(X_l\): \[\frac{T(l,N)}{N^{mn}}=\prod_{i=1}^{l}[1-(1-[(1-\frac{1}{N}\lfloor \frac{N}{p_i} \rfloor)^n(\sum_{a=0}^{k-1}\lfloor\frac{N}{p_i^a}\rfloor_nH_a)])^m]\]

	Interesting: \url{https://en.wikipedia.org/wiki/Legendre%27s_formula} 
