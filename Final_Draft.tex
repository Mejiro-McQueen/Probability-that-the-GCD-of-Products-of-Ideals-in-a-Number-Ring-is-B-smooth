...
% Also illustrates how properties of smooth numbers can be used to crack weak cryptography schemes and creating strong hashing functions, primality tests, and error correcting functions.

%% Core Citations and Extra link above are now placed in the Bibliography!
...
\todo[inline, color=red]{Move to subsection and after altnerative proof, since this depends on either.}
...
\todo[inline, color=red]{Cite where we can find more background and details as needed here!}
...
\begin{lemma} If $k$ is a positive integer, then we have for all sufficiently large $n$
	...
	\todo[color=red, inline]{Where $\epsilon$ is as in the Marcus estimate; cite!}
\end{lemma}
...
\todo[inline, color=yellow]{Review LDCT!}
...
\bibitem{Bai} S. Bai, Polynomial Selection for the Number Field Sieve. 2011. Australian National University, PhD dissertation. https://maths-people.anu.edu.au/~brent/pd/Bai-thesis.pdf
